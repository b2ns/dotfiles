...
(defun random-name ()
  (let ((n (random 10)) (name (make-array 10 :element-type 'character :fill-pointer 0 :adjustable t)))
    (cond
      ((= n 9) (setf n 3))
      ((and (> n 3) (< n 9)) (setf n 1))
      (t (setf n 2)))
    (format name "~a" (nth (random *length-first-name*) *first-name*))
    (loop repeat n do
        (format name "~a" (nth (random *length-last-name*) *last-name*)))
    name))
...
第五	言	福))
(defparameter *length-first-name* (length *first-name*))
...
(defparameter *length-last-name* (length *last-name*))
